...
from helper_chat import add_assistant_message, add_user_message, chat
from helper_report_builder import generate_prompt_evaluation_report
...
class PromptEvaluator:
    ...
    def generate_dataset(
        ...
        if not force_regenerate and Path(output_file).exists():
            try:
                with open(output_file, "r") as f:
                    dataset = json.load(f)
                if isinstance(dataset, list) and len(dataset) > 0:
                    print(
                        f"Loaded existing dataset from {output_file} "
                        f"({len(dataset)} test cases). "
                        f"Pass force_regenerate=True to overwrite."
                    )
                    return dataset
                print(
                    f"Existing {output_file} is empty or invalid; regenerating dataset."
                )
            except (json.JSONDecodeError, OSError) as e:
                print(f"Could not load {output_file} ({e}); regenerating dataset.")
